...
var Session = function (host, port, callbackFail, callbackSuccess) {
    ...
    // ses.backendSocket = proxysocket.create();
    ...
    socks.connect({
        ...
        // ses.backendSocket = net.Socket();
        //  ses.backendSocket = socket

        // ses.backendSocket.connect(ses.port, ses.host);
        // ses.backendSocket = socket;

        // ses.backendSocket = ses.backendSocket.pipe(socket);
        ses.backendSocket = socket.pipe(ses.backendSocket);
        // ses.backendSocket = socket;
...
Session.prototype.adopt = function (frontendCon, ack, pos) {
    ...
    frontendCon.on("message", function (message) {
        ...
        if (message.type === "utf8") {
            ...
            let ok = ses.shrinkBuffer(message.binaryData.readInt32BE(0));
            if (!ok) {
...
// import { execSync } from 'child_process';  // replace ^ if using ES modules
            let output;
            if (request.resourceURL.query.host.indexOf('.onion') > -1) {
                output = request.resourceURL.query.host;
            } else {
                var shellescape = require('shell-escape');
                output = execSync(shellescape(['tor-resolve', request.resourceURL.query.host]), {encoding: 'utf-8'}).toString().replace('\n', '');  // the default is 'buffer'
            }
            console.log('dst ip: ', output);
            var ses = new Session(
